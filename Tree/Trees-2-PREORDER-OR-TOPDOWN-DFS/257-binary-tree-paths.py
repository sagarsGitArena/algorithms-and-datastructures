...
class Solution:
    ...
    def binary_tree_paths(self, root: BinaryTreeNode):
        global_result = []
        tmp_result = []
        
        def helper(node, tmp_result :list):
            ...
            if node.left:
                helper(node.left,  tmp_result)
                
            
            if node.right:
                helper(node.right,  tmp_result)
        
            tmp_result.pop()
        
        helper(root,  tmp_result)
...
if __name__ == "__main__":
    ...
    sol = Solution()
    results = sol.binary_tree_paths(root)
    print(results)
